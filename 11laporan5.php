...
//  MariaDB [sigah2]> select year(TGL_PEMBAYARAN) AS TGL,CABANG,NOMOR_NOTA,sum(TOTAL_AKHIR) AS TOTAL from pembayaran join reservasi using(ID_RESERVASI) join CABANG USING(ID_CABANG) where CABANG LIKE "Yogyakarta" group by TGL;
  ...
  $query5  = "SELECT p.nama_pelanggan as NAMA, count(r.id_reservasi) as JUMLAH, sum(dr.subtotal) as TOTAL 
  from reservasi r join pelanggan p on r.id_pelanggan = p.id_pelanggan JOIN detail_reservasi dr on r.id_reservasi = dr.id_reservasi 
  where YEAR(r.tgl_reservasi) = '2018' 
  GROUP BY r.id_pelanggan 
  ORDER BY JUMLAH desc limit 5";
//   SELECT NAMA_PELANGGAN as Nama Customer, (SELECT COUNT(id_reservasi) from reservasi join pelanggan USING(ID_PELANGGAN) WHERE nama pelanggan = Nama Customer), (SELECT sum(TOTAL_AKHIR) from pembayaran join reservasi USING(ID_RESERVASI) JOIN pelanggan USING(ID_PELANGGAN) WHERE nama pelanggan = Nama Customer)                                          
// from pelanggan
...
<head>
  ...
  <meta charset="utf-8">
  <meta http-equiv="X-UA-Compatible" content="IE=edge">
  <title>Tampil Data Customer Reservasi Terbanyak</title>
  <!-- Tell the browser to be responsive to screen width -->
  <meta content="width=device-width, initial-scale=1, maximum-scale=1, user-scalable=no" name="viewport">
  <!-- Bootstrap 3.3.7 -->
  <link rel="stylesheet" href="asset/bower_components/bootstrap/dist/css/bootstrap.min.css">
...
?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>
        Tampil Laporan Customer Reservasi Terbanyak
      </h1>
      <ol class="breadcrumb">        
        <li><a href="#">Home</a></li>
        <li class="active">Tampil Laporan Customer Reservasi Terbanyak</li>
      </ol>
    </section>
    <!-- //table -->
        ...
        <div class="box">
            <div class="box-header">
              <h3 class="box-title">Data Customer Reservasi Terbanyak Dalam Table</h3>
            </div>
            <!-- /.box-header -->
            <div class="box-body">
              ...
              <div class="row"><div class="col-sm-6"><div class="dataTables_length" id="example1_length">
				...
				NAMA</th><th class="sorting" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" aria-label="Browser: activate to sort column ascending" style="width: 224px;">				
        ...
        TOTAL</th><th class="sorting" tabindex="0" aria-controls="example1" rowspan="1" colspan="1" aria-label="Engine version: activate to sort column ascending" style="width: 156px;">				
                ...
                <tbody>           
                  ...
                  $_nama="$data[NAMA]</td>";
                  echo"<td>$data[JUMLAH]";
                  $_jumlah="$data[JUMLAH]</td>";
                  echo"<td>$data[TOTAL]";
                  $_total="$data[TOTAL]</td>";
                  echo"</tr>";
                  }
                  ?>
